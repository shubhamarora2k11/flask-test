...
class File(db.Model):
    ...
    file_status = db.Column(db.String(50))
...
class NewDocForm(FlaskForm):
	select = SelectField('Document Type', choices=[('invoice', 'Invoice'), ('contract', 'Contract')])
...
def home():
	#inp_dict = {"ids": list(map(str, [1, 2]))}
	#data = json.dumps(inp_dict)
	#flash("Welcome","success")
	files = File.query.all()

	filteronfilestatus = File.query.filter(File.file_status=='Processing').all()
...
def docdetails(doctype):
	...
	if form.validate_on_submit():
		...
		file_object = File(file_id=file_id, file_name=file_name, file_type=file_type, file_status='Processing', file_base64=file_base64)
...
@app.route('/watcher/<int:fid>/<string:fstatus>', methods = ['GET','POST'])
def watcher(fid, fstatus):
	file = File.query.filter(File.file_id==fid).first()
	file.file_status = fstatus
	db.session.commit()
